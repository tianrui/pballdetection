...
import numpy as np
import os, json, cv2, random, pathlib
...
def load_eager_od_data(n):
    datadir = "./dataset"
    dictslist = utilities.get_dataset_for_detectron2(n, datadir)
    ...
    for i in range(n):
        ...
        width = dictslist[i]["width"]
        annotationlist = dictslist[i]["annotations"]
...
def poolball_test():
    ...
    category_index = {duck_class_id: {'id': duck_class_id, 'name': 'pool ball 1'}}
    ...
    test_images_np = []
    for i in range(n, 50):
      image_path = os.path.join(test_image_dir, 'img_' + str(i) + '.png')
